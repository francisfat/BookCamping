...
public class PrenotaCampServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//------------------------------------------
		try {
			// Effettuo il parsing per lavorare con le date
		...
		if ( ok == true) {
			...
			pren.setId(0);
			pren.setCheckin(arrivo);
			pren.setCheckout(partenza);
			...
			currentsession.setAttribute("pren", pren);		// prenotazione piazzola non nulla
			currentsession.setAttribute("pren1", null);		// prenotazione escursione nulla
															// in tal modo il riepilogo capisce
															// che si tratta di una prenotazione di una piazzola
